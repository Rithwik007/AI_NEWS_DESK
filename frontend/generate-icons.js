...
// Draw editorial newsprint emblem
function drawNewsIcon(x, y, w, h) {
  ...
  // Rounded emblem plate background (dark ink)
  const pad = 0.08;
  ...
  // Inside plate: draw folded newspaper/bulletin lines
  // Top header bar (Signal green accent)
  if (ny >= 0.22 && ny <= 0.27 && nx >= 0.28 && nx <= 0.72) {
    return SIGNAL;
  }

  // Headline bar (Deep ink)
  ...
  // Small editorial stamp badge (bottom right of paper)
  const bx = nx - 0.62;
  const by = ny - 0.68;
